...
float player_1_p, player_1_dp, player_2_p, player_2_dp; // position and velocity for both players
...
simulate_player(float* p, float* dp, float ddp, float dt) {
	...
	*dp = *dp + ddp * dt;
	...
	// collision detection
	if (*p + player_half_size_y > arena_half_size_y) { // top frame collision
		*p = arena_half_size_y - player_half_size_y;
		*dp = 0;
	}
	else if (*p - player_half_size_y < -arena_half_size_y) { // bottom frame collision
		*p = -arena_half_size_y + player_half_size_y;
		*dp = 0;
	}
}
...
simulate_game(Input* input, float dt) {
	...
	if (current_gamemode == GM_GAMEPLAY) {  // Gamemode is Gameplay
		...
		float player_1_ddp = 0.f;  // acceleration for player 1
		...
		if (!enemy_is_ai) {
			...
			//if (ball_p_y < player_1_p - 2.f) play  er_1_ddp -= 1300;
			player_1_ddp = (ball_p_y - player_1_p) * 100;
			if (player_1_ddp > 1300) player_1_ddp = 1300;
			if (player_1_ddp < -1300) player_1_ddp = -1300;
		}

		float player_2_ddp = 0.f;  // acceleration for player 2
		...
		// Simulate ball
		{
			...
			// ball collision with player boards
			if (aabb_vs_aabb(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 80, player_1_p, player_half_size_x, player_half_size_y)) { // ball_p_y - ball_half_size?
				ball_p_x = 80 - player_half_size_x - ball_half_size;
				ball_dp_x *= -1;
				ball_dp_y = (ball_p_y - player_1_p) * 2 + player_1_dp * 0.75f;
			}
			else if (aabb_vs_aabb(ball_p_x, ball_p_y, ball_half_size, ball_half_size, -80, player_2_p, player_half_size_x, player_half_size_y)) { // ball_p_y - ball_half_size?
				ball_p_x = -80 + player_half_size_x + ball_half_size;
				ball_dp_x *= -1;
			...
			// ball collision with frames behind players -> ball restarts from the middle
